...
import com.iot.restfulAPI.userlogon;
	  ...
	  public Response perform_request() throws Exception {
	    ...
	    try {
	      ...
	      switch(call_type) {
	        ...
	        case "POST": {
	         //判断是否是登录取accesstoken
	        if(!call_string.toLowerCase().contains("token"))
	        {
	        	//替换accesstoken
	        	JSONObject newbodyjson = new JSONObject();
	        	newbodyjson.put("accessToken", userlogon.accessToken);
	        	//String msgBody = StringEscapeUtils.escapeJava(body);
	        	//加密消息体，构成content
	        	String content = ContentMsg.AESEncrypt(body);
	        	newbodyjson.put("content",content);
